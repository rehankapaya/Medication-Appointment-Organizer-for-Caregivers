...
const getInitialFormData = (): Omit<Patient, 'id' | 'medications' | 'appointments' | 'healthRecords'> => ({
    name: '',
    age: 0,
  ...
  useEffect(() => {
    if (patientToEdit) {
      setFormData({
        ...
        avatarUrl: patientToEdit.avatarUrl || getInitialFormData().avatarUrl,
        personalInfo: patientToEdit.personalInfo || getInitialFormData().personalInfo,
        medicalHistory: patientToEdit.medicalHistory || [],
        allergies: patientToEdit.allergies || [],
        notificationSettings: patientToEdit.notificationSettings || getInitialFormData().notificationSettings,
      });
    } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const patientData: Patient = {
      ...(patientToEdit || { 
          ...
          appointments: [],
          healthRecords: [],
          notificationSettings: getInitialFormData().notificationSettings,
      }),
      ...formData,
    };
    onSave(patientData);
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={patientToEdit ? 'Edit Patient Profile' : 'Add New Patient'}>
      <form onSubmit={handleSubmit} className="p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <FormRow label="Age">
                <input type="number" name="age" value={formData.age || 0} onChange={handleChange} className="w-full p-2 border border-slate-300 rounded-md text-lg" required />
            </FormRow>
